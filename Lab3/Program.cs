...
app.MapGet("/api/renters", [Authorize] async (ModelDB db) => await db.Renters!.ToListAsync());
app.MapGet("/api/priceList/{apartType}", [Authorize] async (ModelDB db,string apartType) => await db.PriceLists!.Where(u=>u.ApartType==apartType).FirstOrDefaultAsync());
